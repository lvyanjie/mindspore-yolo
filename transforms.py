...
def _preprocess_true_boxes(true_boxes, anchors, in_shape, num_classes, max_boxes, label_smooth,
    ...
    if wh.size != 0:
        ...
        boxes_max = wh / 2.
        boxes_min = -boxes_max    # 统一标准，将0作为中心点
        ...
        for index in range(len(flag[0])):
            t = flag[0][index]  # 前一个
            n = flag[1][index]  # next
            if iou[t][n] < iou_threshold:  # 前四大且需要满足阈值
                continue
            for l in range(num_layers):
                if n in anchor_mask[l]:    # anchors_mask
                    ...
                    c = true_boxes[t, 4].astype('int32')  # label
        ...
        for t, n in enumerate(best_anchor):
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    i = np.floor(true_boxes[t, 0] * grid_shapes[l][1]).astype('int32')  # grid_y   # true boxes x y已经进行了normal
                    j = np.floor(true_boxes[t, 1] * grid_shapes[l][0]).astype('int32')  # grid_x
    ...
    # y_true 存放 基于grid坐标 boxes偏移以及label
...
class PreprocessTrueBox:
    ...
    def __call__(self, im, anno, input_shape):
        # bbox_true_1, bbox_true_2, bbox_true_3: 对应三层的 anchor boxes（基于gt偏移）
        # gt_box1, gt_box2, gt_box3: 加入pad之后的bbox，防止动态shape
        ...
        return im, anno, np.array(bbox_true_1), np.array(bbox_true_2), np.array(bbox_true_3), \
...
def _choose_candidate_by_constraints(max_trial, input_w, input_h, image_w, image_h, jitter, box, use_constraints):
    ...
    candidates = [(0, 0, input_w, input_h)]  # 原始输入尺寸是候选1？
...
def _data_aug(image, box, jitter, hue, sat, val, image_input_size, max_boxes,
    ...
    new_image = Image.new('RGB', (input_w, input_h), (114, 114, 114))
...
def preprocess_fn(image, box, config, input_size, device_num):
    ...
    max_boxes = config.max_box  # 数量
    num_classes = config.num_classes
    jitter = config.jitter
    hue = config.hue
    sat = config.saturation
    val = config.value     # 不解其意
